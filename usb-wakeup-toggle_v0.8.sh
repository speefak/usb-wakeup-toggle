...
# licence       : (CC) BY-NC-SA
# version       : 0.8
# notice        : erweitert mit Dialogauswahl, Udev-Funktion und globalem Wakeup-Toggle
# infosource    : ChatGPT
...
toggle_all_devices () {
	...
	for i in "${!DeviceMap[@]}"; do
		...
		if [[ "$current" != "$target" ]]; then
			...
			echo "✔ $DevicePath: bereits $target"
		fi
	done
	echo -e "\n$Green✔ Alle USB-Geräte auf \"$target\" gesetzt.$Reset"
}
#------------------------------------------------------------------------------------------------------------
show_main_menu () {
	dialog --clear --backtitle "USB Wakeup Konfiguration" \
	--title "Aktion wählen" \
	--menu "Was möchtest du tun?" 15 60 5 \
	1 "Einzelne Geräte toggeln" \
	2 "Alle Geräte auf enabled setzen" \
	3 "Alle Geräte auf disabled setzen" \
	4 "Abbrechen / Beenden" 2>menu_choice.txt
